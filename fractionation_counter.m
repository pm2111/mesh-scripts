function [score]= fractionation_counter(filename)
data = load(filename);
...
%      single_deflections = neg_deflection_start==1 & neg_deflection_end==1;
 ...
 for k=1:size(data.(fields{1}).nodes,1)
    ...
    % single_neg_deflections_end_ind = find(single_deflections ==1);
     ...
     fractionation(k) = frac_dummy;
     amplitudes_scaled(k) = max(amplitudes)/max(peak2peak);
     
 end
 
fractionation_positive = fractionation >1;
scar = amplitudes_scaled <0.3 & fractionation_positive >0;
score = [];
for i=1:size(scar,2)
    if scar(i)>0
        score(i) = 2;
    else
        score(i) =fractionation_positive(i);
    end
end
