...
H_mesh = read_CHASTE( 'D:\ARVC meshing automatic\patients\patient09\chaste_gmsh\HEART');
...
data = load('D:\ChrisReconstructions\ARVC9\ARVC9_Baseline.mat');
...
H1 = read_VTK(  'D:\ARVC meshing automatic\patients\patient09\HEART.vtk'); %our target heart
...
u = loadv( 'D:\ARVC meshing automatic\patients\patient09\mpp\Hmapping' , 'u' ); %transfer matrix u
...
write_VTK(H_mesh,'D:\ARVC meshing automatic\patients\patient09\pat09_labelled_ecgi.vtk','b');
...
H_mesh.xyz = H_mesh.xyz/10;
fields{1} = 'septum';
mkdir 'D:\ARVC meshing automatic\patients\patient09\regions';
...
% end
 ...
 fid = fopen(strcat('D:\ARVC meshing automatic\patients\patient09\','regions_pat09_in_mesh_vtk_ordering','.txt'),'w');
    fprintf(fid,'%5.0f \n',[mesh_cell_property].');
    fclose(fid)

    
 fid = fopen(strcat('D:\ARVC meshing automatic\patients\patient09\','epi_cell_property','.txt'),'w');
    fprintf(fid,'%5.0f \n',[epi_cell_property].');
    fclose(fid)
...
regional_cond_lv =[];
    for i=1:20
        ...
        elseif isnan(GadRV(j)) ==0
            regional_cond_lv(j-20*(patient_nr-1)+1) = cond(GadRV(j));
        else
...
%corresponding to the region number next to it
    fid = fopen(strcat('D:\ARVC meshing automatic\patients\patient09\','regions_full','.txt'),'w');
    fprintf(fid,'%d %d %d %d \n',[H_mesh.xyz, regional_cond_lv(mesh_cell_property)'].');
    fclose(fid)
    

H_mesh.xyzlabels = mesh_cell_property;
write_VTK(H_mesh,'D:\ARVC meshing automatic\patients\patient09\sim_mesh_labelled.vtk','binary')
