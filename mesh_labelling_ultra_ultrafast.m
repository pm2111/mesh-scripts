...
mesh = read_CHASTE('D:\ARVC meshing automatic\patients\patient05\chaste_gmsh\HEART');


region_names = dir('D:\ARVC meshing automatic\patients\patient05\aha regions revised\');

filename = 'D:\ARVC meshing automatic\patients\patient05\aha regions revised\';
...
for i=1:size(region_names,1)-2
    ...
    regions_complete{1}(:,2:4) = regions_complete{1}(:,2:4) /10;%%% transform labelled indicies from mm into cm, like the chaste coords
 ...
 labelled_all(:,2:4) =labelled_all(:,2:4)/10; %%%% transform labelled indicies from mm into cm, like the chaste coords
...
[idx,dist] = knnsearch(labelled_all(:,2:4),mesh.xyz(:,1:3)); %find the index in labelled_all which is closesst to point in uncategorised indices 
...
node_group_full_mesh = [];
for i =1:size(mesh.xyz)
    
        belongs_to =  find(regions_known_list_idx ==indices_original_mesh(i));
        group = regions_known_list_number(belongs_to(1));
%         regions_known_list_idx(end+1) = i;
%         regions_known_list_number(end+1) = group(1);
%         
        node_group_full_mesh(i) = group(1);
    if rem(i,100000) ==0
        i
    end
%     
...
% 
% [region_idx_unique, idx_old,idx_new] = unique(regions_known_list_idx);
% regions_group_unique = regions_known_list_number(idx_old);

mesh.xyzregions = transpose(node_group_full_mesh);

%patch('vertices', mesh.xyz, 'faces', mesh.tri, 'facecolor', 'interp', 'cdata', mesh.atts, 'edgecolor', 'none') ;


write_VTK(mesh, 'D:\ARVC meshing automatic\patients\patient05\HEART_volumetrized_labelled_final2.vtk','binary');

mesh= read_VTK('D:\ARVC meshing automatic\patients\patient02\HEART_volumetrized_labelled.vtk');

save('D:\ARVC meshing automatic\patients\patient02\regions tetra.mat','node_group_full_mesh');

dummy = load('D:\ARVC meshing automatic\patients\patient02\regions tetra.mat');
