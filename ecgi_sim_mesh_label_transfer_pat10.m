...
enableVTK;
patient_nr='10';
mkdir 'D:\ARVC meshing automatic\patients\patient10\regions';
...
H_mesh = read_CHASTE( strcat(patient_sim_folder,    'chaste\HEART'));
...
% end
% 
%  fid = fopen(strcat('D:\ARVC meshing automatic\patients\patient05\','regions_pat05_in_mesh_vtk_ordering','.txt'),'w');
%     fprintf(fid,'%5.0f \n',[mesh_cell_property].');
%     fclose(fid)
% 
% 
...
%corresponding to the region number next to it
    fid = fopen(strcat('D:\ARVC meshing automatic\patients\patient',patient_nr,'\','regions_full','.txt'),'w');
    fprintf(fid,'%d %d %d %d \n',[H_mesh.xyz, regional_cond_lv(mesh_cell_property)'].');
    fclose(fid)
    

H_mesh.xyzlabels = mesh_cell_property;
write_VTK(H_mesh,strcat('D:\ARVC meshing automatic\patients\patient',patient_nr,'\sim_mesh_labelled.vtk'),'binary')
