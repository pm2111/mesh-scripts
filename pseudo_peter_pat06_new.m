...
if Eikonal_control==1
     ecg=load('D:\ARVC meshing automatic\patients\patient06\results\Eikonal\eikonal06_fine13.0x_0f_0.3fiber_NEW_pred_ATMap_.mat');
    ecg=ecg.ecg_full;
    Icon=ecg(:,1);
    IIcon=ecg(:,2);
    IIIcon=ecg(:,3);
...
if Eikonal_strong==1
    ecg=load('D:\ARVC meshing automatic\patients\patient06\results\Eikonal\ecg200SmallFibStrongSlowFibers.mat');
    ecg=ecg.ecg_full;
    Is=ecg(:,1);
    IIs=ecg(:,2);
    IIIs=ecg(:,3);
...
if Eikonal_light==1
   ...
   ecg= csvread('D:\ARVC meshing automatic\patients\patient06\results\Eikonal\eikonal06_fine_pred_PseudoECG_200_2x_1f.csv')';
    end 
   %ecg=load('D:\ARVC meshing automatic\patients\patient06\results\Eikonal\ecg100SmallFibLight.mat');
    %ecg=ecg.ecg_full;
    
    [I,II,III,aVR,aVL,aVF,V1w,V2w,V3w,V4w,V5w,V6w]=ecgi_ECG_sim(ecg);
else
load('D:\ARVC meshing automatic\patients\patient06\results\TestHeart06Node2LGE\ecg.mat');
I=I_real;
II=II_real;
...
if Eikonal_int==1
    ecg=load('D:\ARVC meshing automatic\patients\patient06\results\Eikonal\ecg150_RootNodesControlIntermediate.mat');
    ecg=ecg.ecg_full;
    Iint=ecg(:,1);
    IIint=ecg(:,2);
    IIIint=ecg(:,3);
...
ampl=max(real(V3e_f(offset:offset+200)));
ampl_min=min(real(V3e_f(1:600)));

ampl_prev= max(V3ws);
ampl_prev_min=min(V3ws);
...
grid on
if lge==1 && lge2==1
    legend('sim no LGE','sim lge light', 'sim lge intermediate','sim lge strong + eikonal','sim lge very strong','recorded ECG');
else
