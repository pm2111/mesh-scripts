...
camlight
hold on;
scatter3(mesh.xyz(roots,1),mesh.xyz(roots,2),mesh.xyz(roots,3),205,'filled','r')
caxis([0 20])
 axis off
 axis equal
...
new_root_coordsLV2=[8.657 -7.214 -0.3969];
...
[~,id2]=min(distance(mesh.xyz,new_root_coordsLV2));
...
roots_new=[roots(1) roots(2:4) roots([6:7]) idRV];

dlmwrite([data_path new_path '/' fileName_final '_fine_rootNodes7.csv'],roots,'precision',10);

figure()
patch('vertices',mesh.xyz,'faces',mesh.face,'FaceVertexCData',act_bid,'facecolor','interp','edgecolor','none');
camlight
hold on;
scatter3(mesh.xyz(roots_new,1),mesh.xyz(roots_new,2),mesh.xyz(roots_new,3),205,'filled','r')
caxis([0 20])
camlight
