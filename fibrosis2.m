...
for k=1:size(filenames,1)-2
    ...
    for j=1:size(potentials,1)
        y = potentials(j,:);
        dy_dt = diff(y)/dt;
        local_min_idxs = islocalmin(y);
        local_max_idxs = islocalmax(y);
        min_amplitudes = y(local_min_idxs);
        max_amplitudes = y(local_max_idxs);
        dummy = linspace(1,size(potentials,2),size(potentials,2));
        local_min_idxs = dummy(local_min_idxs);
        ...
        min_idx_selected = [];
        max_idx_selected = [];
        for i=2:size(min_amplitudes,2)-1
%             min_qualified = 0;
%             max_qualified =0;
...
%             end
% 
            if abs(min_amplitudes(i))-baseline > 0.3*potential_amplitudes(j)  %check if minima are large enough
              counter = counter+1;
              min_idx_selected = [min_idx_selected,local_min_idxs(i)];
            end
