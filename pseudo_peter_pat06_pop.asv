...
for z=[1]
    if z==1
        pop_path='D:\ARVC meshing automatic\patients\patient06\results\Eikonal\exp24_ECGs\';
    else
...
for i = 3:nb
    ...
    legend('advanced phenotype control ECG','mild phenotype control ECG')
...
highest_scoring=find(tot_score>3);
...
for i =highest_scoring
    ...
    keep_models=vertcat(keep_models,ecg);
        leg_names{l}=string(long_params{l}(1:end-53));
...
ylabel('S wave duration [ms]','FontSize',18)
