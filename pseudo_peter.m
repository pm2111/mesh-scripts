...
enableVTK;
sim_name='TestHeart06Node2LGE2';
pat_nr='06';
try
    load(strcat('D:\ARVC meshing automatic\patients\patient',pat_nr,'\results\',sim_name,'\ecg.mat'));
catch
...
N=size(FBfine.xyz,1);
names = dir(strcat('D:\ARVC meshing automatic\patients\patient',pat_nr,'\results\',sim_name,'\voltages/'));
[ordered,idx]=sort({names.name});
names = names(idx);
ecg_full=[];
for n=1:19
    for i=1:10
       ...
       fid=fopen(strcat('D:/ARVC meshing automatic/patients/patient',pat_nr,'/results/',sim_name,'/voltages/',names(i+3+(n-1)*10).name),'r'); 
