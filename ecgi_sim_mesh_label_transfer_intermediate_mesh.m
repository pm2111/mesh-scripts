...
H_mesh = read_CHASTE( 'D:\ARVC meshing automatic\patients\patient05\chaste_tiny\HEART');
...
data = load('D:\ChrisReconstructions\ARVC5\ARVC5_Baseline.mat');
name = fieldnames(data);
fields = fieldnames(data.arvc5_baseline.labels);
for i=1:size(fields,1)
    indices = data.(name{1}).labels.(fields{i,1});
    H0.xyzlabels(indices) = i;
end
...
%find the nodes in the RV1 (endocardium) which are 5mm or further from the
...
septal_indices_rv = find(distances_rv_epi >6);
...
for i=1:size(unique(mesh_cell_property))-1
    ...
    fid = fopen(strcat('D:\ARVC meshing automatic\patients\patient05\',num2str(fields{i}),'.txt'),'w');
    fprintf(fid,'%d %d %d\n',[H_mesh.xyz(ind,:)].');
    fclose(fid)
end
...
%corresponding to the region number next to it
    fid = fopen(strcat('D:\ARVC meshing automatic\patients\patient05\','regions_full_tiny','.txt'),'w');
    fprintf(fid,'%d %d %d %d \n',[H_mesh.xyz, rand(size(H_mesh.xyz,1),1)].');
    fclose(fid)
    
%save only the coords whose conductivity values are altered
dummy=regional_cond_lv(mesh_cell_property);
save_ind= find(dummy) <1);

    fid = fopen(strcat('D:\ARVC meshing automatic\patients\patient05\','regions_full_cond_change','.txt'),'w');
    fprintf(fid,'%d %d %d %d \n',[H_mesh.xyz(save_ind,:), dummy(save_ind)'].');
    fclose(fid)
