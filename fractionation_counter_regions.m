function [fractionation,amplitude_scaled]= fractionation_counter(filename)
data = load(filename);
...
amplitude_scaled =cell(1,size(labels,1));
...
%             stats(j,4) = max_dphi_dt_int(j) >= 0.05*median(max_neg_gradient);
        ...
        amplitude_scaled{1,z}(end+1) = max(amplitudes)/max(peak2peak);
    end
end
