...
enableVTK;
TORSO_MODEL_DIR = 'D:\ARVC meshing automatic\patients\patient05\chaste_gmsh\';
SUBJECT_DIR = 'D:\ARVC meshing automatic\patients\patient05\';
...
H0 = read_VTK('C:\Users\petnov\Dropbox\shared\MeshPersonalizationPipeline\TORSO\HEART.vtk');
...
H1_full = read_CHASTE(  'D:\ARVC meshing automatic\patients\patient05\chaste_gmsh\HEART'); %our target heart
...
%check that the root nodes preserve position after the initial mesh is
%distorted
figure()
plotMESH( HD ,'ne');
hplot3d( HD.xyz( LV0_roots ,:) , 'o1kb20' );
hplot3d( HD.xyz( RV0_roots ,:) , 'o1kr20' );
headlight; axis(objbounds);
axis off
...
try 
    root_nodes1= load(strcat(patient_sim_folder,'\root nodes\start.txt'));
    LV1_roots = root_nodes1(1:4)+1;
    RV1_roots = root_nodes1(5:7)+1;
catch
    fprintf('no root node file found\n');
end
...
% 2) it must be moving around the LV endocardium towards the septum,
% hence the distance between it and a RV rootnode 3 must be decreasing

dist = 0.25*apexbase_dist<distances &  distances < .26*apexbase_dist;
dist_ind = find(dist);
...
score = dist+z_height+further_away+lies_on_lv_endo;
...
%conditions
%1)Distances between the new root node and old root node need to be 0.485 x
%apexbase dist
%2) The z -height of the candidate root node must be the same as the
%original
%3) The new root node must be closer to the rv node 1 than the previous
%root node
%move it a set distance, say 48.5% of the apexbase vector magnitude
...
%the new root node has to be closer to RV node 1

dist_node2 = [repmat(H1iz.xyz(RV1_roots(1),: ),size(H1iz.xyz,1),1)]-H1iz.xyz(:,:);
dist_node2 = sqrt(dist_node2(:,1).*dist_node2(:,1)+dist_node2(:,2).*dist_node2(:,2)+dist_node2(:,3).*dist_node2(:,3));

prev_dist_RV1_RV2_nodes = H1iz.xyz(RV1_roots(1))-H1iz.xyz(RV1_roots(2));
...
%2) the x and y coords are similar to the previous root node 3
%3) the new root node is on the endocardium
%4) the new node is on the same vertical plane as the old root node

z_height = H1iz.xyz(LV1_roots(3),3)+0.26*apexbase_dist> H1iz.xyz(:,3) &  H1iz.xyz(:,3)> H1iz.xyz(LV1_roots(3),3)+apexbase_dist*0.25;
...
distances = [repmat(H1iz.xyz(LV1_roots(3),: ),size(candidates,1),1)]-H1iz.xyz(candidates,:);
distances = sqrt(distances(:,1).*distances(:,1)+distances(:,2).*distances(:,2)+distances(:,3).*distances(:,3));
...
%now lets move the apical LV root node 2 vertically towards the base
%conditions:
%1) its going to move vertically towards the base by 40%of the apexbase distance
    % ie the distance between the old node and the new node is going to be
    % 25% of apexbase dist
%2) the new root node is on the endocardium
%3) new node needs to preserve x-y coords
...
y_change = abs(H1iz.xyz(:,2)-repmat(H1iz.xyz(LV1_roots(2),2),size(H1iz.xyz,1),1));
x_change = abs(H1iz.xyz(:,1)-repmat(H1iz.xyz(LV1_roots(2),1),size(H1iz.xyz,1),1));

total_change = x_change +y_change;
xy_change = x_change < 5 &y_change <5; 

score = dist+lies_on_lv_endo+xy_change;
candidates = find(score ==3);

distances = [repmat(H1iz.xyz(LV1_roots_new(3),: ),size(candidates,1),1)]-H1iz.xyz(candidates,:);
distances = sqrt(distances(:,1).*distances(:,1)+distances(:,2).*distances(:,2)+distances(:,3).*distances(:,3));
...
fid = fopen(strcat(SUBJECT_DIR,'\root nodes\permuted.txt'),'w');
