...
%         figure(1);
    ...
    key_out = {'I'; 'II';'V1'; 'V2'; 'V3'; 'V4'; 'V5'; 'V6'};
    layout = fliplr(reshape(1:8,4,2))';
    %figure('Position',[0 0 500 1000]); %left bottom width height
  % colours = {'b-.','r--','g--','k:','r--','g--','b:','r:'};
  colours={'b','g', 'k','r','r','m','k','y','c'};
    colours={'b',[0.54 0.82 0.99], 'g',[0.91 0.41 0.17],'r','m','k','y','c', [0.55 0 0 ]};

    handles = [];

    % Grid: Horizontal: large unit = 200 ms, small unit = 40 ms
    %       Vertical:   large unit = 0.5 mV, small unit = 0.1 mV
    x_large_div = 200;            % 100 ms
    y_large_div = 1;            % 0.5 mV
    x_small_div = x_large_div;  % 50 ms
    ...
    for simu = 1
        ...
        for n=1:8
            ...
            if individual_figures
                ...
                x = mod(n,9)-1; % 0 based
                y = ceil(n/9);  % 1 based!
                subplot('Position',[x/12+n/500 row_cum_sum(y) 1/12 row_height_rel(y)]);
...
%             else
            if ischar(colours{simu})
                ...
                plot(ts,ECGs{n},'LineWidth',3,'color',colours{col});
                ...
                if(~individual_figures)
                    font_size = 25;
                    text(40, maxs(1)-0.5, key_out(m,:), 'FontSize', font_size);
                else
                    font_size = 0.1*7;
