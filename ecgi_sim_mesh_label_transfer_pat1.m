...
patient_sim_folder =  'D:\ARVC meshing automatic\patients\patient01\';

%H0 = read_VTK( fullfile( TORSO_MODEL_DIR , 'HEART_with_UPS_louie.vtk' ) ); %this is louie's heart!
H_mesh = read_CHASTE( 'D:\ARVC meshing automatic\patients\patient01\chaste\HEART');
...
data = load('D:\ChrisReconstructions\ARVC1\ARVC1_Baseline.mat');
...
H1 = read_VTK(  'D:\ARVC meshing automatic\patients\patient01\HEART.vtk'); %our target heart
...
u = loadv( 'D:\ARVC meshing automatic\patients\patient01\mpp\Hmapping_ecgi' , 'u' ); %transfer matrix u
...
write_VTK(H_mesh,'D:\ARVC meshing automatic\patients\patient01\pat01_labelled_ecgi.vtk','b');
...
%corresponding to the region number next to it
    fid = fopen(strcat('D:\ARVC meshing automatic\patients\patient01\','regions_full','.txt'),'w');
    fprintf(fid,'%d %d %d %d \n',[regional_cond_lv(mesh_cell_property)'].');
    fclose(fid)
    
        fid = fopen(strcat('D:\ARVC meshing automatic\patients\patient01\','mesh_cell_property','.txt'),'w');
    fprintf(fid,'%d \n',[mesh_cell_property].');
    fclose(fid)
