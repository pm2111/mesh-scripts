...
H0 = read_VTK('D:\ARVC meshing automatic\patients\patient06\HEART_0.70.vtk');

meshpath='D:\ARVC meshing automatic\patients\patient06\eikonal06_coarse\';
% 
% xyzs=csvread(strcat(meshpath, 'eikonal06', '_coarse_xyz.csv'));
% tris=csvread(strcat(meshpath, 'eikonal06', '_coarse_tri.csv'));
% lvface=csvread(strcat(meshpath, 'eikonal06', '_coarse_lvface.csv'));
% rvface=csvread(strcat(meshpath, 'eikonal06', '_coarse_rvface.csv'));
% epiface= csvread(strcat(meshpath, 'eikonal06', '_coarse_epiface.csv'));
% 
% 
% H0=struct;
% H0.xyz=xyzs;
% H0.tri=tris;
% H0.face=vertcat(epiface ,lvface,rvface);
...
roots06=dlmread('D:\ARVC meshing automatic\patients\patient06\eikonal06_coarse_rootNodes5_RV_anterior_5_RV_posterior_3.csv');

LV0_roots = roots06(1:4);
RV0_roots = roots06(5:end);


% 
% meshpath='D:\ARVC meshing automatic\patients\patient09\eikonal09\';
% 
% xyzs=csvread(strcat(meshpath, 'eikonal09', '_fine_xyz.csv'));
% tris=csvread(strcat(meshpath, 'eikonal09', '_fine_tri.csv'));
% lvface=csvread(strcat(meshpath, 'eikonal09', '_fine_lvface.csv'));
% rvface=csvread(strcat(meshpath, 'eikonal09', '_fine_rvface.csv'));
% epiface= csvread(strcat(meshpath, 'eikonal09', '_fine_epiface.csv'));
...
H1 = read_CHASTE( 'D:\ARVC meshing automatic\patients\patient06\chaste06_edge_0.15_RV_0.2\HEART'  );
%H1 = read_VTK(  'D:\ARVC meshing automatic\patients\patient06\HEART_0.30.vtk'); %our target heart
...
u = loadv( strcat(SUBJECT_DIR, '\mpp\Hmapping6coarse2thin') , 'u' ); %transfer matrix u
...
figure()
plotMESH( HD ,'ne');
    patch('Faces',H1.face,'Vertices',H1.xyz,'EdgeColor','none');
headlight; axis(objbounds);
axis off
end
...
dlmwrite(strcat(SUBJECT_DIR,'start09.txt'),vertcat(LV1_roots,RV1_roots))

dlmwrite(strcat(SUBJECT_DIR,'\eikonal06_edge_0.1_RV_0.2\eikonal06_edge_0.1_RV_0.2_coarse_rootNodes.csv'),[LV1_roots;RV1_roots]','delimiter',',','precision',10);

H_mesh2 = read_CHASTE( strcat(patient_sim_folder,    'chaste06_coarse','\HEART'));


roots_pos=[sept1.Position; sept2.Position;sept3.Position;sept4.Position;sept5.Position;sept6.Position];
idx=[];
for i=1:size(roots_pos,1)
    idx(i)=knnsearch(H_mesh2.xyz,roots_pos(i,:)/10);
    %rootsNew=vertcat(rootsMod(1:4)',idx,rootsMod(5),rootsMod(7));
   % dlmwrite([filePath '/' fileName '_fine_rootNodes_RV_anterior_',num2str(i),'.csv'],rootsNew,'precision',10);

end

for i=1:6 %post
      rootsNew=vertcat(LV1_roots(1:4),RV1_roots(1:2),idx(i)); %loc 5 is anterior RV

     dlmwrite(strcat(SUBJECT_DIR, '/roots_coarse/', 'eikonal06' ,'_coarse_rootNodes_RV_sept_', num2str(i),'.csv'),rootsNew,'precision',10);
    
end


      rootsNew=vertcat(LV1_roots,RV1_roots); %loc 5 is anterior RV

figure()
    patch('Faces',H1.face,'Vertices',H1.xyz,'EdgeColor','none');
headlight
hold on
    scatter3(H1.xyz(rootsNew,1),H1.xyz(rootsNew,2),H1.xyz(rootsNew,3),135,'filled','r')
colorbar
